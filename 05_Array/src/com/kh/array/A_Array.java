...
public class A_Array {
	...
	public void method5() {
		...
		arr = null;	// 참조형 변수에 null값을 대입하면, 
					// 가비지 컬렉터가 해당 메모리를 즉시 삭제시켜준다.
		//System.out.println(Arrays.toString(arr));
	...
	public void method7() {
		...
		char[]arr2 = new char[str.length()]; //length string의 글자수
		
		for(int i = 0; i<arr2.length; i++) {
			arr2[i] = str.charAt(i);
		}
		
		System.out.println(arr); //문자를 붙여서 출력
		System.out.println(Arrays.toString(arr)); // arrays.toString() : 는 구조를,로 나누어 출력
