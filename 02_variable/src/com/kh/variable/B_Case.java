...
public class B_Case {
	 ...
	 * 	  자료형 변수명 = 값;
	 * 	  서로 다른 자료형끼리 연산을 수행하고 싶으면 한 쪽을 형변환해야함.
	 * 
	 * 형변환의 종류
	 ...
	 * 
	 * */
	public void autoCasting() {//run파일에서 선언해줘야함.
		...
		long l4 = /*(long)*/ 2000; //int는 -21억~21억이므로 그 이상값은 l을 붙여줌
		// 다만 너무 큰 리터럴 값을 사용해야 하는 경우 l을 붙여줌.
		
		//특이케이스 3. long -> float
		//long==> 8byte, float ==> 4byte
		//정수가 실수에 담길때에는 큰 사이즈의 정수여도 작은 사이즈의 실수에 대입 가능.
		//실수는 지수표기법을 사용하기 때문에 long자료형보다 담을 수 있는 값의 범위가 훨씬 크다.
		long l5 = 10000000000L;
		float f5 = /*(float)*/ l5;
		
		// 특이케이스 4. char <-> int
		//양방향 형변환 가능(int가 양의 정수일때)
		
		char ch = /*(char)*/ 65; //아스키 코드표의 범위 (0~127)
		System.out.println("ch : "+ch);
		
		int num = '정'; //한글과 같은 기타나라의 언어는 유니코드표(0~65xxx)에 기술되어있다.
	...
	public void forceCasting() {
		//1. double -> int
		double d1 = 10.89;
		int i1 = (int)d1;	//(int)를 쓰지 않으면 강제 변환이 일어나지 않음
		// 10.89 -> 10
		System.out.println("i1: "+i1);
		
		//2. double(대표자료형) -> float
		float f1 = (float)4.0; //강제 형변환
		float f2 = 4.0f; // long = 대문자L, float = 소문자
		float f3 = (float) d1;
