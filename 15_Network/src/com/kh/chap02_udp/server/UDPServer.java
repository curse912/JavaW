...
public class UDPServer {
	/*
	 * UDP
	 * 	- 비연결형 프로토골로 클라이언트가 데이터를 받았는지 아닌지 상광하지 않는 전송방식
	 * 	- 데이터 손실이 발생 할 수 있다.
	 * 	- 번거로운 확인절차를 없앰으로써 신속하게 데이터를 전달할 수 있다.
	 * 	- 빠른 데이터 전송이 중요한 실시간 스트리밍, 게임 등에서 사용된다.
	 * 
	 * DatagramSocket
	 * 	- UDP통신에서 DatagramSocket을 보내고, 받는데 사용하는 클래스
	 * 	- DatagramSocket을 수신하거나 전송할 수 있으며, 클라이언트와 서버에 각각 하나의 DatagramSocket을 
	 * 	  만들어두고 이를 통해 송수신함.
	 * 
	 * DatagramPacket
	 * 	- UDP통신시 데이터를 포장하는 클래스.
	 * 	- 전송할 데이터화 함께 호스트의 IP주소 및 포트번호를 포함하여 보낸다.
	 * 
	 * Datagram
	...
	public static void main(String[] args) {
		...
		try {
			...
			socket.receive(packet);	//클라이언트가 넘겨받은데이터를 packet에 복사
			...
			socket.send(packet2);	//클라이언트가 receive중이라면 데이터를 수신가능
