...
public class A_StringPool {	//문자열 상수풀
	//String 불변클래스( 변하지 않는 클래스 )
	//String 은 기존에 담겨있던 값이 수정되는 순간 우너래 담겨있던 공간(주소값)에서 수정되지 않는다.
	//String 은 새로운 값이 추가되는 순간 새로운 저장공간이 할당된다.
	
	//문자열 상수 풀(String Constant Pool)
	//	- 문자열 데이터가 담기는 영역으로, heap메모리 영역상에 고정적으로 할당되어 있다.(jdk7 이상)
	...
	public void method1() {
		...
		System.out.println(str1.hashCode());	// 상수풀의 주소값으로 사용	//Object클래스내에있음
