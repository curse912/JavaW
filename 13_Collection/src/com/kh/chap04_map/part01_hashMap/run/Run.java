...
public class Run {
	public static void main(String[] args) {
		/*
		 * HashMap ?
		 * 	- 많은 양의 데이터를 "빠르게 저장하고 관리" 하기위하여 key,value값을 함께 묶어서 관리하는 자료구조.
		...
		// 1. put( K key, V value) : map 공간에 key, value를 세트롤 추가하는 매서드
		...
		// 2. get(K Key) : V => Hashmap에서 key와 일치하는 value값을 찾는 매서드.
		...
		// 4. replace( K key, V value) => hashmap에서 key값과 일치하는 값을 찾아, 새로 전달된 value로 교체하는 매서드.
		hm.replace("포테이토칩", new Snack("피자맛",1000));
		System.out.println(hm);
		
		// 5. remove( K key ) => hashmap에서 key값과 일치하는 값을 찾아서 삭제 해주는 매서드(key + value 함께 삭제)
		hm.remove("칸초");
		System.out.println(hm);
		...
		// hashmap을 반복시키는 방법
		// 1.keySet()을 활용한 반복
		//	- hashmap의 key값들을 따로 Set에 담아서 변환해주는 메서드
		...
		// 2. entrySet()을 활용한 반복
		//	- hashmap의 key,value값이 담긴 entry를 Set에 담아서 반환해주는 매서드
