...
public class HashMapRun <K,V>{
	...
	class Entry<K,V>{	///class내의 접근제어자(public)없는 class는 class내에서만 사용되는 class임 
		K key;
		V value;
		Entry<K, V> next;
	...
	public int getIndex(K key) {
		// 매개변수로 전달받은 key을 활용하여 table에 저장할 인덱스 위치를 구하려는 함수
	...
	public void put(K key, V value) {
		...
		if (table[index]==null) {
			...
			if(table[index].key.equals(newEntry.key)) {
				...
				return;	//size가 추가 되지 않도록 종료.
			}else {
				// "다이제", "칸초", "칸초" => 우연히 hashcode 중복으로 충돌이 발생한경우
				//	- 다이제가 저장된 entry의 next에 칸초를 저장
				
				
				// "다이제" , "칸초" , "새우깡" 모두 같은 인덱스를 공유하는 경우
				
//				table[index].next = newEntry;	//얘 어디서 났지...
		...
		// 만약 next에도 이미 값이 존재하는 경우, next가 없는 entry를 찾을때까지 반복
	...
	public V get(K key) {
		...
		if(e == null) {
			...
			do {
				if(e.key.equals(key)) {
					return e.value;	//entry에 제너릭을 하지 않으면 object값을 받아서 오류가 나옴
				}
				e = e.next;
			}while(e != null);
		}
