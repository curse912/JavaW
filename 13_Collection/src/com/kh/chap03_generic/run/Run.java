...
public class Run {
	public static void main(String[] args) {
		...
		for(Object o : l) {
			System.out.println((String)o);	//o에 담긴 데이터가 문자열이 아닐수도 있다.
		}
		
		/* 2. 제네릭(JDK 1.5버전에서 제네릭 등장)
		 	- 데이터의 자료형을 일반화 시켜서 여러 자료형을 다룰 수 있게 만든 문법
		 	- 클래스의 메서드의 반환형, 매개변수, 필드의 자료형들을 "미리 지정하지 않고" 파라미터화 하여 
			  나중에 사용할 수 있게 만드는 방법.
			- 제네릭을 사용하면 다양한 타입을 저장할 수 있기 때문에 "타입변수"라고 불린다.
			- 제네릭을 사용하면 런타임 시점이 아닌 "컴파일" 시점에서 에러를 캐치할 수 있으므로 "타입 안정성"
			  을 확보 할 수 있다.
		 */
		Generic<Integer, String> g1 = new Generic<>();
		g1.setT(1111);	//컴파일 시점에서 T는 Integer로 확정
		g1.setG("String");
		g1.printTest("String");
		g1.printTest(12345);
		g1.<String>printTest("1111");
		
		Generic<String,Integer> g2 = new Generic<>();
//		g2.setT(1111);	//g2에서 T는 String형 자료형
