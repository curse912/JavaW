...
public class B_Switch {
	 ...
	 * swith문
	 * - if문과 동일한 조건문
	 * - if문은 조건식을 복잡하게 기술할 수 있음.
	 * 	 하지만, switch은 확실한 하나의 값만 기술(동등비교만 수행가능)
	 * 
	 * [표현법]
	 * 	switch(동등비교할 조건식, 변수) {
	 * 	case 값1 : 실행할 코드;	// switch문의 변수와 값1이 동등한 경우 실행하는 코드
	 * 			   break;	// 코드 실행 후 switch문을 탈출하는 역할 연산자
	 * 				//break가 없다면 아래쪽으로 코드 순차적으로 실행.(다음 case로 넘어간당)	
	 *  case 값2 : 실행할 코드;	
	 * 			   break;
	 * 	case 값n : 실행할 코드;	
	 * 			   break;
	 *	...
	 *	
	 * 	default : //위의 값들 중 일치하는 값이 없는 경우 실행할 코드를 기술
	 * 				break; //생략가능
	 * 	}
	 * 
	 * switch의 장점 ->case를 통해 내가 실행하기를 원하는 토드로 즉시 이동하기 때문에 연상속도가 if ~ else if문보다 빠르다.
	 * 
	 * switch문의 사용처
	 * 	- 항상 동일한 속도로 연산이 수행되어야 하는경우
	 * 	  ex) 키보드 입력, 마우스 입력
	 * 	- 가독성 측면에서 if ~ else if문보다 더 뛰어난 경우
	 * 	  ex) 동등비교 조건식을 5회 이상 사용하는 경우 switch 사용 권장
	 * 
	 * 
	 * */
	...
	public void method2() {
		...
		case "망고" : case "바나나" : 
			...
			break;
		default :
			System.out.println("저희 가게에서 판매하는 과이이 아니네요");
			return; //메서드 종료
		}
