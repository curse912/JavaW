...
public class A_For {
	 ...
	 * 			조건식의 결과가 false라면 반족 종료.
	 * - 증감식 : 반복문을 제어하는 변수 값을 증감시키는 구문(++,--)
	 * 
	 * 	for문 실행
	 * 	1. 초기식 실행
	 * 	2. 조건식 실행
	 * 	3_1. 조건식 결과가 참이라면 for문 내부코드 실행
	...
	public void method4() {
		...
		int sum2 = 0; // 1부터 10까자ㅣ의 값을 저장할 변수
		
		for (int i = 1; i <= 10; i++) {
			//반복문 안에 변수 초기화할 경우, 반복때마다 0이됨.
			sum2 += i;	
		}
	...
	public void method6() {
		...
		for (int i = 0; i < str.length(); i++) { //변수.length() : 글자 수
	...
	public void method9 () {
		...
		//중첩반복문으로 별찍기
		/*
		for (int j = 1; j<=4; j++) { 		// y축
			for(int i = 1; i<=4; i++) {		// x축
