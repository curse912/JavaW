...
public class OuterClass3 {
	...
	public Runnable getRunnable (int num) {
		...
		/* 지역 내부 클래스
		 * 	- 지역변수처럼 매서드 내부에서 클래스를 정의하여 사용하는 것
		 * 	- 매서드 안에서만 사용가능하며, 매서드가 호출될때 생성된다..
		 * 	- 지역 내부클래스안에서 사용되는 지역변수는 '상수(final)'처럼 취급된다.
		 * 									( 메모리 효율성 ) 
		 * 
		 * 메모리복습
		 * 1) stack
		 * 	- 메서드 호출시 스택에는 메서드프레임이 쌓인다.	
		 * 	- 매서드 프레임 내부에는 지역변수를 위한 저장공간이 할당.
		 * 	- 메서드 종료시에는 메서드프레임이 제거되면서 지역변수의 저장공간도 함꼐 소멸된다.
		 * 
		 * 2) static 키원드가 붙은 변수는 static메모리 영역에 저장공간이 할당
		 * 	  static final(상수) -> static메모리 내부의 constant pool에 저장공간이 추가
		 * 	  final 키워드만 붙은 변수 -> static메모리 내부의 constant pool에 저장공간이 할당 	//contatnt pool 
		 * 
		 */
		final int local = 1;	//지역변수
//		local = 2;	// final이 붙어서 값을 바꿀수 없음.
		...
		class localInnerClass implements Runnable{
			...
			@Override
			public void run() {
				System.out.println(local);	// 매서드 소멸시 지역변수의 저장공간도 함께 소멸
				// local = 2;
