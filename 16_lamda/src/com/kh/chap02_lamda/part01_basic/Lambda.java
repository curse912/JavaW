...
public class Lambda {
	 ...
	 * 
	 * 함수형 프로그래밍(FP)
	 * 	- 함수를 기반으로 코드를 작동하고, 함수를 호출하여 동작하는 프로그램은 만드는 방식
	...
	public static void main(String[] args) {
		...
		/*
		 * LambdaTest1인터페이스에 들어갈 객체는 반드시 LambdaTest1 인터페이스를
		 * 상속받아, 추상 메서드를 구현한 클래스여야한다.
		 * 익명 내부 클래스의 문법처럼 이름 없는 클래스의 형태로 객체를 생성해야하지만, 불필요한 
		 * 코드작성을 줄이는 방식으로 간소화 되었다.
		 * 
		 * 함수형 인터페이스는 고정적으로 1개의 추상메서드만 보관하고 있으므로,
		 * 구현해야할 함수의 이름, 반환형, 매개변수의 자료형 등은 생략이 가능
		 * */
		
		LambdaTest1 lambda2 = (x,y) -> {return x+y;};	// ()->{} : 객체이다.
		lambda2.add(5, 5);
		System.out.println(result);
